...
# Note: EfficientNetB0 is no longer imported
...
# --- [All other configuration constants (paths, image size, etc.) are the same] ---
...
BATCH_SIZE = 64
EPOCHS = 100 # May need adjustment when training from scratch
LEARNING_RATE = 0.1 # A higher learning rate is often better for training from scratch
RANDOM_STATE = 42
PATIENCE_EARLY_STOPPING = 15
PATIENCE_REDUCE_LR = 7

OUTPUT_DIR = os.path.join('./', f"run_{MODEL_NAME}_{IMG_WIDTH}_{BATCH_SIZE}_DataAugmentation_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
# Data Augmentation (this layer is still very useful)
...
# %%
# --- 2. Model Architecture: CUSTOM CNN FROM SCRATCH ---
print("\nPhase 2: Building a Custom CNN Model From Scratch")

inputs = Input(shape=(IMG_WIDTH, IMG_HEIGHT, 3))

# Normalize pixel values to be between 0 and 1. This is crucial.
x = Rescaling(1./255)(inputs)

# --- Start of the Custom Convolutional Base ---
...
x = BatchNormalization()(x)
x = MaxPooling2D(pool_size=(2, 2))(x)
# --- End of the Custom Convolutional Base ---

# --- Start of the Classifier Head (same as before) ---
x = GlobalAveragePooling2D(name="avg_pool")(x)
x = Dropout(0.5, name="top_dropout_1")(x) # Increased dropout rate as the model might overfit more easily
...
# %%
# --- 4. Training the Model (Single Phase) ---
print("\nPhase 4: Training the Custom Model")
# There is no feature extraction or fine-tuning, just a single training loop.
...
# Assuming train_dataset and val_dataset are created and not empty
history = model.fit(
    train_dataset,
    epochs=EPOCHS,
