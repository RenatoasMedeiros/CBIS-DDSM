...
OUTPUT_DIR = os.path.join('./', f"run_{MODEL_NAME}_{IMG_WIDTH}_{BATCH_SIZE}_BinaryCrossentropy_SGD_With_Momentum_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
optimizer_fine_tune = SGD(learning_rate=LEARNING_RATE / 10) # Use a smaller LR
