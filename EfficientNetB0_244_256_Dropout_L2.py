...
OUTPUT_DIR = os.path.join('./', f"run_{MODEL_NAME}_{IMG_WIDTH}_{BATCH_SIZE}_Dropout_L2_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
print("\nPhase 2: Building the Model with L2 Regularization")
...
x = Dropout(0.3, name="top_dropout_1")(x) # Technique 1: Dropout

# Add L2 regularization to this Dense layer
x = Dense(128,
          activation='relu',
          kernel_regularizer=l2(0.001), # <-- TECHNIQUE 2: L2 Regularization
          name="dense_128")(x)

x = Dropout(0.3, name="top_dropout_2")(x) # Technique 1: Dropout
